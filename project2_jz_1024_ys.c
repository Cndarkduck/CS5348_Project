...
int initfs(char* path, unsigned short blocks,unsigned short inodes) {
   ...
   for (i = 0; i < superBlock.isize; i++){
     lseek(fileDescriptor, BLOCK_SIZE*(2 + i), SEEK_SET);//YS: mistake corrected
...
void create_root() {
  ...
  lseek(fileDescriptor, root_data_block*BLOCK_SIZE, 0); //YS: Mistake corrected
...
int cpin(char *externalfile, char *v6_file){
	...
	while(1){
		...
		if(fileRead < BLOCK_SIZE){
			...
			printf("Finish copying external file %s to the file %s. The size of the file is %d.\n", externalfile, v6_file, totalSize);
			break;
		}
		addrIndex++;
	}
