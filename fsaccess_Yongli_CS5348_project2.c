...
int allocinode() {
    ...
    if(superBlock.ninode == -1) {
        ...
        while (superBlock.ninode < I_SIZE) {
            lseek(fileDescriptor, 2 * BLOCK_SIZE + i * INODE_SIZE, SEEK_SET);
            read(fileDescriptor, &freeinode, INODE_SIZE);
            if ((freeinode.flags & 0100000) == 0) { //unallocated
                superBlock.ninode++;
                superBlock.inode[superBlock.ninode] = i;
                //lseek(fileDescriptor, 1 * BLOCK_SIZE + 2 + 2 + 2 + 4*FREE_SIZE + 2 + superBlock.ninode * 4, SEEK_SET);
                //write(fileDescriptor, &i, 4);
            }
            i++;
        }
    }
...
int get_inode_number(char* filename, int inode_number) {
    ...
    dir_type dir_entry;
    int offset;
    lseek(fileDescriptor, 2 * BLOCK_SIZE + inode_number * INODE_SIZE + 12, SEEK_SET);
...
int find_position_of_dir_entry_for_this_file(char *inpath){
    ...
    dir_type dir_entry;
    int offset;
    lseek(fileDescriptor, 2*BLOCK_SIZE + j*INODE_SIZE + 12, SEEK_SET);//+ point to addr[0] of parent directory
    ...
    for (int k = 0; k< BLOCK_SIZE/DIR_SIZE; k++)
    {
        ...
        {
            printf("The file: %s is at inode#: %d\n",new_name,i);
            clear_inode(i);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void clear_inode(int index) //TODO: start number adjustment
{
    char emptyb[BLOCK_SIZE] = {0};
    lseek(fileDescriptor, 2*BLOCK_SIZE + index*INODE_SIZE, SEEK_SET);
    read(fileDescriptor, &inode, INODE_SIZE);
    int i = 0;    
    while(inode.addr[i]!=0){
      clear_block(inode.addr[i]);
      i++;
    }
...
int cpin(char *externalFile, char *v6File) {
    ...
    while (read(exFileFd, &buffer, BLOCK_SIZE) != 0){
        ...
        if (indexOfAddr < ADDR_SIZE) {
            lseek(fileDescriptor, 2 * BLOCK_SIZE + newFileInodeNum * INODE_SIZE + 12 + indexOfAddr * 4, SEEK_SET);
...
int cpout(char* internal_path, char* external_path){
    ...
    printf("The inode number of %s is %d\n", internal_path, inode_no);
    int count =0;
    ...
    //inode_type inode;
    lseek(fileDescriptor, 2*BLOCK_SIZE+ inode_no*INODE_SIZE, SEEK_SET);
    read(fileDescriptor, &inode, INODE_SIZE); // inode is internal_path's inode
    char buffer[1024];
